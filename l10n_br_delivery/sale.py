        ...
        for order in self.browse(cr, uid, ids):
            for invoice in order.invoice_ids:
                if invoice.state in ('draft') and order.fiscal_operation_id:
                    ...
                    if not doc_serie_id:
                        raise osv.except_osv(_(u'Nenhuma série de documento fiscal !'),_(u"Não existe nenhuma série de documento fiscal cadastrada para empresa:  '%s'") % (order.company_id.name,))
